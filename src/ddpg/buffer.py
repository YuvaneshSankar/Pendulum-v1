...
class Buffer:
    def __init__(self,max_size,action_dim,state_dim):
        self.maz_size=max_size
        self.ptr=0 #pointer to the current posiion in buffer
    ...
    def store(self,state,action,reward,next_state,done):
        ...
        self.ptr=(self.ptr+1)%self.maz_size
        self.size=min(self.size+1,self.maz_size)

    def sample(self,batch_size):
        ind=np.random.randint(0,self.size,size=batch_size)
        return dict(state=torch.FloatTensor(self.states[ind]),
